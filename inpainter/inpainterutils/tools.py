...
def log_startup_info(dataset_size, config):
    niter = int(config['train']['n_epochs'] * dataset_size / config['train']['batch_size'])
    msg = '\n\n\n'
    msg += 'DATASET SIZE: {}\n'.format(dataset_size)
    msg += 'TOTAL ITERATION COUNT: {} (how many times the model is trained on some image)\n'.format(niter)
    msg += '\n\n\n'
    print(msg)
    return niter

def pil_loader(path, img_format='L'):
...
def random_bbox(config, batch_size):
    ...
    """
    img_height, img_width, _ = config['train']['image_shape']
    h, w = config['train']['mask_shape']
    margin_height, margin_width = config['train']['margin']
    maxt = img_height - margin_height - h
    maxl = img_width - margin_width - w
    bbox_list = []
    if config['train']['mask_batch_same']:
...
def mask_image(x, bboxes, config):
    height, width, _ = config['train']['image_shape']
    max_delta_h, max_delta_w = config['train']['max_delta_shape']
    mask = bbox2mask(bboxes, height, width, max_delta_h, max_delta_w)
    if x.is_cuda:
        mask = mask.cuda()

    if config['train']['mask_type'] == 'hole':
        result = x * (1. - mask)
...
def spatial_discounting_mask(config):
    ...
    """
    gamma = config['train']['spatial_discounting_gamma']
    height, width = config['train']['mask_shape']
    shape = [1, 1, height, width]
    if config['train']['discounted_mask']:
...
# PLURALISTIC INPAINTING TOOLS

# VALIDATION TOOLS
def diversity_lpips(images):
    """
    get average diversity of a group of images (e.g. inpaintings) with
    LPIPS
    """
    if images.shape[0] < 2:
        # undefined for just one img
    ...
    # get all combinationso
    all_pair_idxs = list(combinations(range(images.shape[0]), 2))

    score_tot = 0
    for pair_idxs in all_pair_idxs:
        img0 = images[pair_idxs[0]].unsqueeze(dim=0) # add batch dim
        img1 = images[pair_idxs[1]].unsqueeze(dim=0)
        #print((img0.shape), (img1.shape))
        score = loss_fn_alex(img0, img1) # higher means more different  
        score_tot += score

    avg_score = score_tot / len(all_pair_idxs)
    return avg_score

def quality_critic(images, critic):
    """
    get average quality (discriminator/critic score)
    of a set of images
    """

    qualities = critic(images)
    avg_qual = torch.mean(qualities)

    #print('qualities: {}'.format(qualities))
    #print('stdev quality: {}'.format(torch.std(qualities)))

    return avg_qual

def rectangularmask_tobbox(mask):
    '''
    convert mask (NxCxHxW), N=C=1 to box (top, left, height, width)

    mask MUST be rectangular
    '''
    mask2d = torch.squeeze(mask)
    ...
    for i in range(mask2d.shape[0]):
        for j in range(mask2d.shape[1]):
            if mask2d[i][j] == 1 and not topcornerfound: # if going from top left down and to the right,
                top = i
                left = j
                # first 1 seen will be top corner of box
                topcornerfound = True
            if mask2d[i][j] == 1 and mask2d[i+1][j] == 0 and mask2d[i][j+1] == 0:
                height = i - 1 - top
                width = j - 1 - left
                # bbox found
...
def extract_critic_features(local_inpainting, netD_local):
    # extract output of feature layer in local discriminator for an inpainting
    score = netD_local(local_inpainting)
    print(score)
    features = netD_local.saved_featuremap

    return features

def scale_tsne_results(x):
    rng = np.max(x) - np.min(x)
    start = x - np.min(x)
    return start / rng

def visualize_critic_features(local_inpainting_features, local_groundtruth_features, mode='norms'):
    '''
    visualize feature maps from final pre-linear layer of trained critic/discriminator
    for a list of inpaintings and the corresponding ground truth
    (no surroundings; masked region only_
    '''
    from sklearn.manifold import TSNE
    viz_fname = os.path.join('experiments/featurespace_dis', 'inpainting_featureviz.pdf') 

    # prep features
    features = torch.cat(local_inpainting_features)
    features = torch.flatten(features, start_dim=1)
    gt_features = torch.flatten(local_groundtruth_features)

    if mode == 'tsne':
        features = features.detach().cpu().numpy()
        gt_features = gt_features.detach().cpu().numpy()
        gt_features = gt_features.reshape(1, gt_features.shape[0])
        features_all = np.concatenate((features, gt_features))
        ...
        tsne = TSNE(
                ...
                perplexity=5.,
                learning_rate=500.
                ).fit_transform(features_all)

        tsne_x = scale_tsne_results(tsne[:, 0])
        tsne_y = scale_tsne_results(tsne[:, 1])

        fig = plt.figure()
        plt.scatter(tsne_x[:-1], tsne_y[:-1], label='inpaintings')
        plt.scatter(tsne_x[-1], tsne_y[-1], label='ground truth')
        plt.legend()
        plt.title('t-SNE embedding of inpaintings and ground truth')
        plt.show()
        plt.savefig(viz_fname)
        print('inpainting t-SNE feature plot saved at {}.'.format(viz_fname))

    elif mode == 'norms':
        # compute avg Linfty norm max(abs) between each inpainting and the g.t.
        inf_norms = torch.norm(features - gt_features, p=float('inf'), dim=1)
        #print(inf_norms, torch.mean(inf_norms).item())

        return torch.mean(inf_norms).item()

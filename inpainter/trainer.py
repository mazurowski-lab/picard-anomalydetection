...
class Trainer(nn.Module):
    def __init__(self, config):
        ...
        self.optimizer_g = torch.optim.Adam(self.netG.parameters(), lr=self.config['train']['lr'],
                                            betas=(self.config['train']['beta1'], self.config['train']['beta2']))
        d_params = list(self.localD.parameters()) + list(self.globalD.parameters())
        self.optimizer_d = torch.optim.Adam(d_params, lr=config['train']['lr'],
                                            betas=(self.config['train']['beta1'], self.config['train']['beta2']))

        print('in Trainer(): device_ids = {}'.format(self.device_ids))
        if self.use_cuda:
    ...
    def forward(self, x, bboxes, masks, ground_truth, compute_loss_g=False):
        ...
        self.train()
        # apply custom dropout config not taken care of in model constructor
        # if necessary
        if self.config['netG']['dropout_which'] == 'CUSTOM': 
            customize_dropout(self, self.config)#, verbose=True)
